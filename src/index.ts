...
import { formatGotchas, GOTCHAS } from './curated/gotchas.js';

// Import template generators
import { generatePlayerControllerTemplate } from './templates/input-templates.js';
import {
    generatePickingSystemTemplate,
    generatePrefabSpawnerTemplate,
    generateObjectPoolTemplate,
    generateTagFilterTemplate,
} from './templates/gameplay-templates.js';
import {
    generateAudioManagerTemplate,
...
function generateInputHandlerTemplate(name: string, inputType: string, inputStyle: string = 'direct'): string {
...
const DECORATOR_MAP: Record<string, string> = Object.fromEntries(
    DECORATORS.map(d => [d.name, `@RE.props.${d.name}()`])
);
    ...
    try {
        switch (name) {
            ...
            case 'generate_input_handler': {
                const { name: handlerName, directory, inputType, inputStyle = 'direct' } = args as {
                    name: string;
                    directory: string;
                    inputType: string;
                    inputStyle?: string;
            ...
            case 'add_component_property': {
                const { filePath, propertyName, propertyType, decorator = 'num', defaultValue } = args as {
                    filePath: string;
                    propertyName: string;
                    propertyType: string;
                    decorator?: string;
            ...
            case 'list_builtin_components': {
                const { group, search } = args as { group?: string; search?: string };
            ...
            case 'create_picking_system': {
                const { name: componentName, directory, selectableTag = 'Selectable' } = args as {
                    name: string;
                    directory: string;
            ...
            case 'create_prefab_spawner': {
                const { name: componentName, directory, spawnOnStart = false } = args as {
                    name: string;
                    directory: string;
            ...
            case 'create_audio_manager': {
                const { name: componentName, directory, trackCount = 3 } = args as {
                    name: string;
                    directory: string;
                    trackCount?: number;
                };
            ...
            case 'create_event_manager': {
                const { name: componentName, directory } = args as {
                    name: string;
                    directory: string;
                };
            ...
            case 'create_game_manager': {
                const { name: componentName, directory, includeSceneManagement = true } = args as {
                    name: string;
                    directory: string;
            ...
            case 'create_object_pool': {
                const { name: componentName, directory, initialSize = 10, autoGrow = true } = args as {
                    name: string;
                    directory: string;
                    initialSize?: number;
                    autoGrow?: boolean;
            ...
            case 'create_player_controller': {
                const { name: componentName, directory, includeJump = true, inputStyle = 'action-based' } = args as {
                    name: string;
                    directory: string;
                    includeJump?: boolean;
                ...
                const fileName = `${componentName}.re.ts`;
                const fullPath = path.join(directory, fileName);
                const content = generatePlayerControllerTemplate({ name: componentName, includeJump, inputStyle: inputStyle as 'direct' | 'action-based' });
