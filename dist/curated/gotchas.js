...
export const GOTCHAS = [
    {
        ...
        detail: 'The two names differ by one character and do opposite things. setAssetPath(uuid, path) ' +
...
RE.AssetManager.setAssetPath(uuid, url);`,
    },
    {
        ...
        detail: 'getAssetPath(uuid) is the single funnel for every uuid-keyed asset fetch (prefabs, ' +
...
const obj = await RE.Model.instantiate(name);`,
    },
    {
        ...
        detail: 'A shadow-casting point light renders its shadow map as a cubemap — at least six draw ' +
    ...
    },
    {
        ...
        detail: 'RE accelerates Mesh.raycast with a bounding volume hierarchy automatically, but skinned ' +
    ...
    },
    {
        ...
        detail: 'Since 1.1, a scene or prefab containing a 3D model stores a lightweight stub referencing ' +
    ...
    },
    {
        ...
        detail: 'RE.AudioAsset exposes getAudio() and getPositionalAudio(); it has no play()/stop() of its ' +
...
this.music.getAudio().play(); // or use an AudioPlayer component`,
    },
    {
        ...
        detail: 'three.js moved away from Clock, so RE replaced Runtime.clock with a THREE.Timer exposed ' +
    ...
    },
    {
        ...
        detail: 'RE.Input.getAxes(name, player?) returns a Vector2-like {x, y} for an axes-based action. ' +
...
const v = RE.Input.getAxis("Vertical");`,
    ...
    },
    {
        ...
        detail: 'The dropdown reads its options from an instance property named exactly ' +
...
modeOptions = ["Easy", "Hard"];`,
    },
    {
        ...
        detail: 'awake() runs as soon as the component is activated — asset-backed props may not be ' +
...
export function formatGotchas(topic) {
