 ...
 */
import { ENGINE_CLASSES, BUILTIN_COMPONENTS, ENGINE_DECORATORS, ENGINE_META, metaFooter, } from './engine-api.js';
import { LIFECYCLE } from './curated/lifecycle.js';
...
export const CLASS_INFO = ENGINE_CLASSES;
/** All prop decorators, including the list.* / map.* forms. */
export const DECORATORS = ENGINE_DECORATORS;
/** Every documented symbol name, for tool enums. Derived — never hand-listed. */
export const CLASS_NAMES = Object.keys(CLASS_INFO).sort();
export const COMPONENT_NAMES = Object.keys(BUILTIN_COMPONENTS).sort();
function groupClasses() {
    const core = [], input = [], assets = [];
    const rendering = [], utils = [], fns = [];
    ...
    for (const [name, ci] of Object.entries(CLASS_INFO)) {
        if (ci.kind === 'functions')
            ...
            input.push(name);
        else if (UTILS.has(name))
            utils.push(name);
        else
            core.push(name);
    }
    const byGroup = {};
    for (const [n, ci] of Object.entries(BUILTIN_COMPONENTS))
        (byGroup[ci.group ?? 'Core'] ??= []).push(n);
    return [
...
export function searchDocs(query) {
    ...
    const results = [];
    const hit = (hay) => hay.toLowerCase().includes(q);
    const anyWord = (hay) => words.some(w => hay.toLowerCase().includes(w));
...
// Formatters
// ---------------------------------------------------------------------------
function renderProps(title, list) {
    if (!list.length)
        return '';
    let out = `\n## ${title}\n\n`;
    for (const p of list) {
...
function renderMethods(title, list) {
    if (!list.length)
        return '';
    let out = `\n## ${title}\n\n`;
    for (const m of list) {
        ...
        if (m.description)
            out += `${m.description}\n\n`;
        if (m.inheritedFrom)
            out += `_Inherited from ${m.inheritedFrom}._\n\n`;
        if (m.example)
            out += '```typescript\n' + m.example + '\n```\n\n';
    }
    return out;
}
function formatEntry(ci) {
    let out = `# ${ci.name}\n\n${ci.description}\n`;
    if (ci.access)
        out += `\n**Access:** \`${ci.access}\`\n`;
    if (ci.baseClass)
...
export function formatClassInfo(className) {
...
export function formatComponentInfo(name) {
...
export function formatComponentList(group, search) {
    let entries = Object.values(BUILTIN_COMPONENTS);
    if (group)
        entries = entries.filter(c => (c.group ?? 'Core').toLowerCase() === group.toLowerCase());
    if (search) {
...
export function formatDecorators() {
    ...
    for (const d of base) {
        out += `## ${d.syntax}\n\n`;
        if (d.propertyType)
            out += `**Property type:** \`${d.propertyType}\`\n\n`;
        out += `${d.description}\n\n`;
        if (d.example)
            out += '```typescript\n' + d.example + '\n```\n\n';
        out += '---\n\n';
