...
export function formatFFClassInfo(className) {
    const info = FF_CLASS_INFO[className];
    if (!info)
        return `Unknown FocusFramework class: ${className}`;
    const lines = [
        `# ${info.name}`,
        ``,
    ...
    if (info.methods?.length) {
        ...
        for (const m of info.methods) {
            ...
            if (m.example)
                lines.push(`\`\`\`typescript`, m.example, `\`\`\``);
        }
    }
...
//# sourceMappingURL=focusframework-data.js.map